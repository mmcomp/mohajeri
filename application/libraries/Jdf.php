...
class Jdf {
    ...
    function jalali_to_jgregorian($j_y, $j_m, $j_d, $mod = '') {
        $j_y = $this->tr_num($j_y);
        $j_m = $this->tr_num($j_m);
        $j_d = $this->tr_num($j_d);
        $doy_j = ($j_m < 7) ? ( ( ($j_m - 1) * 31 ) + $j_d) : ((($j_m - 7) * 30) + $j_d) + 186;
        if ($doy_j > (($j_y % 4 == 3) ? 287 : 286)) {
            $gy = $j_y + 622;
            $gd = $doy_j - (($j_y % 4 == 3) ? 287 : 286);
        } else {
            $gy = $j_y + 621;
            $gd = $doy_j + 79;
        }
        $d_m_g_a = array(0, 31, (($gy % 4 == 0) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31);
        $gdi = 1;
        foreach ($d_m_g_a as $i => $v) {
            if (($gdi - $v) > 0) {
                $gd-=$v;
                $gdi = $gd;
                $gm = ($i + 1);
